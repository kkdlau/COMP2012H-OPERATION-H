...
const QVector<char> Character::Controller::listenKeyList = {'a', 's', 'd', 'w',
															' ', 'k', 'm'};

QHash<char, Character::MOVE_DIRECTION> Character::Controller::KEY2DIRETION = {};

Character::Controller::Controller() { keyStateInitialize(); }
...
void Character::Controller::setState(char key,
									 Character::Controller::KeyState state) {
	if (isalpha(key)) key = tolower(key);
	bool oldState = keyState[key];
	keyState[key] = state;
	if (oldState == false && state == true) updateKeyPressControl();
	//    qDebug() << "set state: " << key << " -> " << pressed;
	updateKeyHoldingControl();
}

void Character::Controller::unControl() { toControl = nullptr; }
void Character::Controller::control(Character *c) { toControl = c; }
...
void Character::Controller::updateKeyHoldingControl() {
	...
	if (v.length()) toControl->moveBy(v.x(), v.y());
}
